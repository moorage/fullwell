...
export function renderWebRoute(url: string, context: WebRenderContext): RenderedWebRoute {
  ...
  const title = pageTitles[pathname] ?? (pathname.startsWith("/c/") ? "Shared collection" : pathname.startsWith("/invite/") ? "Family invitation" : pathname.startsWith("/households/") ? "Household" : "Fullwell");

...
Account mutations remain server-owned. Apple and email linking complete their provider proof in the same signed-in browser; method removal preserves at least one sign-in path; grant revocation and deletion require recent authentication. A disabled-by-default OpenAI review adapter may resolve one encrypted, operator-configured reviewer identity and issue the ordinary browser session, but it provides no public enrollment, recovery, or reusable password surface and remains hidden when disabled. The OpenAI ownership proof is a separate fixed well-known HTTP route whose bounded value enters through the encrypted deployment credential boundary. Household leave and account deletion commit the former-member document and audit event through the single Git writer under the household lock before Neon membership projection changes. Deletion then revokes OAuth tokens, sessions, passkeys, and external identities while retaining the stable actor ID and a pseudonymous display label.
...
Codex alone starts that local server with a fixed lifecycle flag and loads a packaged hook definition after the user reviews and trusts it. One closed-world lifecycle tool can arm an actual grocery order-detail audit to the exact hashed Codex session and current turn. The hook stores only an opaque run ID, monotonic coordination revision, aggregate order counts, turn ID, and one-use continuation nonce under private `PLUGIN_DATA`; it never reads the transcript or becomes journal authority. `SessionStart(compact|resume)` restores only that bounded context, `Stop` continues only the armed collecting turn, and `UserPromptSubmit` disarms an older real-user turn. Consequently later restock, delivery reorder, meal-planning, and unrelated turns are inert unless they explicitly start or resume a grocery audit. Claude starts the same runtime without the flag, exposes no lifecycle tool, and loads no Codex hook.

Deleting local guest state does not delete or demote an authenticated cloud household. When OAuth context returns an existing membership, the remote context supplies the stable user ID for local draft sharding and the authenticated actor ID for evidence attribution; the client resumes cloud authority without hydrating a new local copy. Ordinary bounded cloud journal updates carry new evidence and their citing items/reports through one `hfj_commit_change_set`. The server validates that prospective state under the household advisory lock and mutates the transaction-scoped Neon projection before completing the Git-backed mutation. Separate `hfj_append_evidence` calls remain for evidence-only checkpoints and migrations.

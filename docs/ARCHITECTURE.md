...
The agent client contains no bundled household data, hosted account state, credentials, Git synchronization engine, or programmatic semantic classifier. Codex and Claude use the same skill source files, one dependency-free plugin-provided `fullwell-local` stdio MCP server, and the same optional remote MCP endpoint. Before cloud connection, one bounded revisioned JSON guest household under the active Codex home is authoritative on the current computer. It is not Git, is not synchronized in the background, and never contains browser or authentication material.

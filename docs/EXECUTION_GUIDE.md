...
DigitalOcean Droplets are amd64, while the default Apple silicon build is arm64. Build a release image with `container build --platform linux/amd64`, verify the recorded variant before transfer, and export that variant with `container image save --platform linux/amd64`. Apple's export is an OCI image index. After `docker load`, identify the loaded index digest with `docker image ls --no-trunc`, tag that digest to a Docker-normalized intermediate name, and then tag the normalized reference to the exact immutable `HFJ_IMAGE` name. Docker 29 can retain an additional unqualified containerd reference that appears in `docker image ls` but is unavailable to Compose. Before changing `deploy.env`, require both `docker image inspect --platform linux/amd64 "$HFJ_IMAGE"` and a bounded `docker run --rm --platform linux/amd64 --entrypoint node "$HFJ_IMAGE" --version` to succeed without a pull.

...
- current local milestone: prepared agent package `1.1.9` replaces version-specific local-household shell commands with the plugin-provided `fullwell-local` server's stable read, update, and collecting-only deletion tools; ordinary onboarding can retain one narrow host permission across compatible package upgrades without a broad Node allow rule, while destructive cancellation remains separately gated
- the stable local-tool package passes all 20 package/lifecycle tests, the 45-case cross-host eval matrix, package validation, 29 WebKit checks with seven intentional skips, and full verification with 289 deterministic application tests and 11 expected database skips; its 20-entry dry pack has SHA-1 `a9f9474924de809d4df879ee81ff11260ca423b2` and SHA-512 `sha512-iS8nSYRT8c4520KM6XwyHMxG7Fz2HB18ghTsaQexkl2I8otY9IEeFRoUuzkPuMgDRsYsWQOWvnE9HXywXGGHBg==`, and it is not yet committed or published

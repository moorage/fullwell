...
- conversational Fullwell onboarding is implemented and deployed server-side with schema `0007`, membership-authorized report-derived completion, per-user start/skip/resume state, a 23-tool MCP surface, shared snack-then-recipe skills, five new cross-host evals, and Codex/Claude install handoff prompts; `@fullwell/fullwell@1.1.0` is prepared and registry publication awaits npm web authentication

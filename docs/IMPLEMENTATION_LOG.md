...
- prepared `@fullwell/fullwell@1.1.6` and the matching server change so one confirmed onboarding write accepts up to 10,000 unique items and 10,000 evidence records inside a route-specific 16 MiB MCP envelope; direct HTTP tools remain at one megabyte, repository capacity is checked before Git, and isolated evidence commits 20,000 paths without expanding them into process arguments; full verification passes 284 tests with 11 expected database-gated skips, and the separate security, load, 29-pass browser, 36-case eval, and eight-test package gates pass

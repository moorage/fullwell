...
- current local milestone: whole-grocery onboarding now models `snack`, `ingredient`, `condiment`, and `other_grocery` items in separate canonical areas, collects them during one order-detail pass, retains below-threshold identities for future source-aware requests, and includes every grocery area in the credential-free restocking snapshot; package `1.1.7`, 39 cross-host eval cases, 300 deterministic tests with 11 expected database skips, and 29 WebKit checks with seven intentional skips pass, while the release remains uncommitted, unpublished, and undeployed

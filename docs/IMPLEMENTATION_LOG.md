...
- current local milestone: prepared agent package `1.1.8` makes a fresh Fullwell installation ask whether the person already has an account before any hosted call; a negative answer creates one private revisioned local guest household for grocery and recipe onboarding, direct restocking, and recipe recall, while optional OAuth/cloud promotion is deferred until the user requests backup, WhatsApp, sharing, or family access

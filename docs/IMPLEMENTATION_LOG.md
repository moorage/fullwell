...
- current milestone: the direct WhatsApp gateway, schema `0006`, authenticated Account linking, fixed-path Git snapshot, macOS runner, Codex/Claude adapters, restocking skill/evals, fake retailer, aggregate operator metrics, encrypted deployment credentials, and zero-paid-message cutoff are implemented; staging runs `hfj-staging:whatsapp-20260721-7-runtime` with master, webhook, linking, and runner claims enabled and service replies rolled back off after Meta rejected the first real response with Graph code `131037`
- conversational Fullwell onboarding is implemented locally with schema `0007`, membership-authorized report-derived completion, per-user start/skip/resume state, a 23-tool MCP surface, shared snack-then-recipe skills, five new cross-host evals, and Codex/Claude install handoff prompts; seven-migration up/down/up, 11 isolated PostgreSQL tests, dual-host lifecycle, security, 273 deterministic tests, 90.07% branch coverage, build, and the 29-check WebKit matrix pass

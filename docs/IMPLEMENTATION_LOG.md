...
- successful local finalization and hosted onboarding now invite the user to try an out-of-stock request such as `We're out of cashews; restock them`, explain that Fullwell uses the usual product and store, and preserve confirmation before any cart addition; unsuccessful and no-grocery runs do not show the invitation

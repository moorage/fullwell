...
- prepared client `1.1.2` so a bare `@Fullwell hi` reads unresolved onboarding state before replying, begins the snack audit rather than generic help, and then advances to recipe questions while reusing saved source preferences

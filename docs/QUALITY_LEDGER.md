...
- Contract, 20-case Codex/Claude eval, isolated Codex CLI 0.144.4 and Claude Code 2.1.123 plugin lifecycle, nine-test local PostgreSQL and managed Neon integration, reversible local migration, idempotent managed-Neon forward migration, account lifecycle, security-boundary, WebKit e2e, Git restore, local deployment, MCP discovery, Docker, Caddy, Compose, and OpenTofu checks pass.
...
- The immutable npm agent package is not yet published; Neon PITR plus real Apple, email, Codex, and Claude interoperability still need non-production credentials and external compatibility evidence.

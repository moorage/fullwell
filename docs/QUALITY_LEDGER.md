...
- Conversational Fullwell onboarding, schema `0007`, per-user start/skip/resume state, report-derived household completion, the 23-tool contract, and the install-to-conversation handoff pass local deterministic, PostgreSQL, dual-host package, security, and responsive WebKit gates. Staging migration and publication of the updated client remain unclaimed.

...
- Contract, 20-case Codex/Claude eval, isolated Codex CLI 0.144.4 and Claude Code 2.1.123 plugin lifecycle, nine-test PostgreSQL integration, reversible migration, account lifecycle, security-boundary, WebKit e2e, Git restore, local deployment, MCP discovery, Docker, Caddy, Compose, and OpenTofu checks pass.
...
- The immutable npm agent package is not yet published; real Neon, Apple, email, Codex, and Claude interoperability still needs non-production credentials and external compatibility evidence.

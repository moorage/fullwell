...
- Conversational Fullwell onboarding, schema `0007`, one-read ephemeral drafting, atomic finalization, per-user skip recovery, report-derived completion, the 24-tool contract, and truthful MCP annotations pass deterministic, PostgreSQL, dual-host package, security, responsive WebKit, staging deployment, and current-Codex start gates. Public `@fullwell/fullwell@1.1.4` is npm `latest`, passes downloaded Codex/Claude lifecycles, is enabled in both current hosts, and presents benefit-first snack and recipe guidance before source questions.

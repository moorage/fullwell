...
- Conversational Fullwell onboarding, schema `0007`, per-user start/skip/resume state, report-derived household completion, the 23-tool contract, and the install-to-conversation handoff pass deterministic, PostgreSQL, dual-host package, security, responsive WebKit, and staging deployment gates. Public `@fullwell/fullwell@1.1.0` is the npm `latest` release and passes a clean registry install plus isolated Codex and Claude lifecycle tests.

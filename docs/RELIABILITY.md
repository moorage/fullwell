...
A guest onboarding run initializes one bounded local household without requiring network or OAuth. Every update compare-and-sets a monotonic revision under a local lock, writes a complete temporary document, atomically renames it, and synchronizes the parent directory before reporting success. Interrupted or competing writes therefore leave either the prior valid revision or the complete new revision, never a success-shaped partial file. Finalizing the local journal makes it immediately usable on that computer; declining cloud backup performs no remote call and does not discard it.

...
Delivery audits advance one user-selected provider and bounded history window at a time. An order group becomes reorder authority only when every declared line, modifier, quantity, location, and fulfillment field is present. Local-to-cloud promotion is independently idempotent per provider, and a lost response reconciles the exact payload before recording cloud linkage.

A delivery cart session binds one complete prior delivery group, current local revision or cloud HEAD, provider origin, restaurant location, fulfillment mode, exact source-line quantities, full parsed cart baseline, requested and preserved subtotals, automatic maximum, and any replacement confirmation. Completion requires a fresh whole-cart proof. After an uncertain browser result, Fullwell re-reads the cart and adds only verified missing deltas; unexpected quantity, modifier, location, fulfillment, unrelated-line, subtotal, authority, or session drift blocks instead of replaying clicks. A later conversation must re-resolve and re-read rather than reuse ephemeral authority. Sign-in, MFA, CAPTCHA, unsupported UI, cross-origin navigation, and age/identity UI fail closed or return control to the user, and no recovery path can check out, pay, tip, change delivery address/schedule, or accept a membership or subscription.

Takeout visual-journal pagination uses the same bounded 12-item cursor and 200-item no-JavaScript prefix as recipes and groceries. Hydrated continuations echo the initial repository HEAD; a changed HEAD returns `REVISION_CONFLICT` and leaves the already-rendered prefix intact with a visible retry/refresh path rather than appending from another snapshot. Server-rendered `?page=N` requests intentionally begin from the current synchronized projection.
...
Content-addressed web assets may use a one-year immutable cache policy. The compatibility path `/assets/fullwell-full-body-tall.png` is intentionally stable while its approved artwork may change, so it must use bounded revalidation and the homepage must carry the current content-version query. Replacing bytes at an immutable stable URL without changing the rendered reference is a release failure because existing browsers may retain the prior artwork indefinitely.
...
Delivery telemetry is limited to low-cardinality safe outcome classes for audit completeness, ambiguity, replacement, uncertainty, retries, and cart-prepared/not-prepared. It never includes provider identity, restaurant/location text, dish/modifier text, order locators, cart contents, or age/identity data.
...
Food-delivery verification must prove complete-group validation, provider-then-location disambiguation, provider-scoped promotion recovery, public projection privacy, meal-source recheck, full-cart proof, exact-quantity recovery, replacement confirmation, age-step isolation, and no-checkout behavior. Fixture evidence does not establish a live named-provider or installed-host support claim.

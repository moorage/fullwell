...
The consent page's Content Security Policy permits a validated native-client redirect only as one exact loopback origin: `http://127.0.0.1:<ephemeral-port>` for either the legacy exact `/oauth/callback` path or a bounded merged-desktop `/callback/<nonce>` path, or `http://localhost:<explicit-port>` for Claude's exact `/callback` path. Other pages do not receive a loopback form destination, and remote, unbound, credentialed, query-bearing, fragment-bearing, missing-port, missing-nonce, extra-segment, or unrelated loopback callback values do not widen the policy.
...
Food-delivery history cataloging is limited to explicit user-directed navigation of order history visible in the user's already signed-in browser at an exact approved HTTPS origin. It does not crawl public pages, run unattended, handle credentials, bypass sign-in/MFA/CAPTCHA, or call undocumented provider interfaces. Grocery, recipe, and delivery item-learning traversals may retain only a credential-free HTTPS image visibly associated with the exact audited item/detail page plus that page's exact HTTPS URL. They do not inspect hidden network traffic or raw HTML, broaden into unrelated image search, store listing thumbnails as provenance, or retain image bytes/screenshots. The delivery journal excludes raw pages, screenshots, delivery destinations, payment data, provider account identifiers, and age/identity documents. Connected contribution requires a provider-specific household visibility and retention preview and one provider per mutation.
...
The authenticated Takeout browser is a separate membership-authorized allowlist projection. It may show public restaurant/location, provider label, bounded prior-order counts/dates, fulfillment modes, recent modifiers, classification, and public import attribution, but never provider origin or private order/group/merchant/menu/evidence/actor/destination fields or a complete order. History summaries fail closed unless every cited delivery evidence record still matches the current item. Continuation requests bind the initial Git HEAD and return a conflict instead of mixing snapshots after a concurrent household update.
...
The temporary OpenAI reviewer adapter is absent unless a complete encrypted username and password are configured and its independent enable gate is true. Its same-origin form is limited to five attempts per client address per 15 minutes, accepts either the exact configured origin or browser Fetch Metadata proving a same-origin document navigation when the global no-referrer policy suppresses `Origin`, compares both submitted values through the constant-time HMAC boundary, stores only a keyed reviewer subject in the identity table, returns one generic failure, and issues the ordinary browser session after success. Explicit cross-site metadata remains rejected. Reviewer credentials, their digests, and the isolated reviewer account identifiers are excluded from Git, logs, metrics, screenshots, Beads, and operator output. Disablement hides the form and route; credential rotation also revokes the reviewer's browser and OAuth sessions. The separate OpenAI ownership challenge is accepted only as a bounded base64url value from the encrypted credential store and is returned solely from its fixed uncached well-known endpoint.

...
- [ ] 2026-07-24T18:37Z: Replace Namecheap's HTTP-only forwarding for `fullwell.ai` and `www.fullwell.ai` with DNS to the existing gateway, add Caddy-owned HTTPS permanent redirects that preserve path and query, and prove the canonical application origin and login surfaces remain unchanged.

...
- [x] 2026-07-28T05:30Z: Pushed commit `d3f6e37`, built the exact `linux/amd64` source into image index `sha256:6ea2850bf25dc7e9af0883b59a45ca13307595de4982dbdc862bea41e2673229`, and deployed `hfj-staging:safari-d3f6e37-runtime` to `fullwell.ai` with the prior image retained for rollback. Public deploy/MCP smokes, schema `0008` readiness, rendered Safari guide copy, and local Safari runner status pass.
...
- [x] 2026-07-27T06:32Z: The 20 compatibility retries had left the request marked queued but ineligible to lease. Repaired that exact staging row, then added a one-time fresh-process recovery claim so a fixed or upgraded runner automatically resets only its own confirmed link's saturated queued work. The live retry reached isolated Codex, resolved one historical item, and stopped without cart mutation because the runner still targets the unavailable local feasibility retailer.
...
- [x] 2026-07-27T06:48Z: Removed only the stale five-day-old pending reply, recovered and processed the new live request through the corrected 3,456-file snapshot, proved isolated Codex resolved one familiar candidate and stopped before cart mutation against the unavailable feasibility retailer, then deployed release commit `9b72f08` as checksum-matched Linux/amd64 image `hfj-staging:whatsapp-recovery-20260726-1-runtime` at OCI index digest `sha256:7f227fd7c2e798ffddcbc2f6784dd003a2e4bd65e1fda349a6214f64d35c3f46`. Public deployment/MCP smokes pass, the rebuilt LaunchAgent is running, and Meta's live phone record remains code-verified with display name `DECLINED`, so service replies remain safely disabled.
...
- 2026-07-27: Meta's current phone status reports the Fullwell display name as `DECLINED`, not the earlier `AVAILABLE_WITHOUT_REVIEW`. Inbound intake and local cart work remain available, but service replies stay disabled because business verification does not override the sending identity's rejected display name.

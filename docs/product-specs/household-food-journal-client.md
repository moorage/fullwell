...
2. choose the `hi` starter shown beside the Fullwell identity or say `@Fullwell hi` in Codex, or say `Hi Fullwell` in Claude;
...
If the answer is yes, the first protected tool starts MCP OAuth. If the person previously removed the saved `fullwell-cloud` credential and restarted Codex, the installed server can remain configured while its tools are absent from that turn. After explicit connect or reconnect consent, and only in that state, the Fullwell chat runs exactly `codex mcp login fullwell-cloud` through Codex's command-execution surface and waits for completion. The command accepts no user-derived server name, URL, scope, flag, or other argument. Codex owns the browser callback and keyring credential; Fullwell never reads or stores it. After command success the client does not yet claim a connection, because the current turn's MCP binding is fixed. It asks for one continuation message, then requires a successful `hfj_get_context` on the next turn before continuing hosted work. A denied, cancelled, unavailable, or failed login makes no authority change and exposes no raw command transcript, authorization URL, callback value, code, or token. Claude continues using its native protected-tool authentication and never runs the Codex command. The service authorization page offers:
...
The browser traverses every qualifying order detail once and collects all grocery areas together. It authors or updates an evidence-backed item for every in-scope identity, including a one-off or otherwise below-threshold purchase. For each exact product row or visible exact product/detail link, it records the visibly associated credential-free HTTPS image plus the exact page URL when available. When Safari Computer Use has no DOM access, an accessibility-tree omission is not evidence that the product lacks an image: the agent targets the exact visible product image, uses only a currently exposed context-menu action whose meaning is `Open Image in New Tab`, validates the credential-free HTTPS address from the temporary tab, retains the original exact product row or detail page as provenance, returns to it, and closes the temporary tab. Listing thumbnails, hidden network traffic, raw HTML, retained screenshots, clipboard contents, guessed actions or URLs, unrelated image searches, HTTP/data/blob/credential-bearing URLs, and unproven decorative or tracking images are not provenance. A missing image never blocks complete textual evidence. Refresh preserves a prior valid image/page pair unless the newly inspected exact page proves a replacement. The recurrence threshold controls report assertions only and must not discard an item needed for a later source-aware request.
...
When supported browser control exposes the displayed image element's URL, the agent reads that visible-page DOM attribute without inspecting raw HTML or hidden traffic. Safari Computer Use has no DOM access, so an omitted accessibility image node and an always-available add-image control are not evidence that the recipe lacks an image. The Safari fallback targets the exact visible recipe image, opens its currently exposed context menu, selects only a visible action whose meaning is `Open Image in New Tab`, reads the credential-free HTTPS address from the temporary tab's address field, retains the original audited recipe page as image-page provenance, returns to that page, and closes the temporary tab. An unavailable or ambiguous image target or menu action, unsafe address, or unprovable association leaves the image null or preserves the prior valid pair without blocking textual evidence. The fallback never clicks add/remove-image controls, retains screenshots, reads clipboard contents, guesses menu actions or URLs, or broadens into unrelated image search.
...
The initial homepage and shared masthead use the user-supplied Fullwell icon, with the masthead retaining the visible `Fullwell` wordmark. The homepage preserves the stable `/assets/fullwell-full-body-tall.png` URL for compatibility even though that URL now serves the canonical icon, and the rendered image reference carries a short content-version query so previously cached artwork is replaced immediately. Because its bytes may change without a path change, that legacy URL must revalidate rather than use the one-year immutable asset policy. Both placements use explicit intrinsic dimensions and decorative alternative text and must remain responsive without delaying, obscuring, or replacing install, sign-in, navigation, or legal controls. The homepage presents the supplied artwork directly and does not synthesize a gradient oval behind it.

The exact supplied icon is Fullwell's canonical application image. Deterministically fitted variants supply the browser favicon, Apple touch icon, and web-app manifest without redrawing the artwork. Public Open Graph and Twitter metadata use a content-versioned 1200x630 Fullwell social card with the exact text `Fullwell`, `Household food AI Agent`, and `Snacks, groceries, meal planning in an agent environment you already use.` The card includes the same icon with equal top and right padding so platform previews do not crop its face or retain stale immutable bytes after an artwork change. Homepage Schema.org identifies the icon as the Fullwell application image and brand logo rather than as the Sous Chef Studio corporate logo. The immutable client package includes the exact canonical bytes and Codex references them through its supported plugin-logo metadata. Claude's strict manifest must not add an icon field unless Claude documents one as supported.
...
After the install action, show one setup prompt. Codex and Claude installation add the `moorage/fullwell` marketplace and then install `fullwell@fullwell`. Codex uses the stable `Fullwell` display name and may expose a `codex://new` action that prefills the installed plugin mention plus `hi`; it must tell the user that the prompt is not sent until they review it and press Send. The manual Codex fallback is `@Fullwell hi`. Claude shows the conversational greeting `Hi Fullwell.` without a Codex deep link. Codex Desktop renders the plugin identity separately, so the manifest exposes exactly three natural-language starter bodies without an embedded identity or mention: `hi`, `i'm out of cashews`, and `reorder from Wanpo in stanford mall`.
...
42. Grocery computer-use collection ignores listing thumbnails, uses the exact visible-image context-menu and temporary-tab address fallback when Safari has no DOM access, stores a credential-free HTTPS image/page pair when available, and commits it with the grocery item.
43. Recipe computer-use refresh uses the exact visible-image context-menu and temporary-tab address fallback when Safari has no DOM access; it rejects unsafe or unproven images, preserves a prior valid pair when no replacement is proven, and never blocks complete textual evidence for a missing image.
44. Delivery computer-use refresh uses the exact visible-image context-menu and temporary-tab address fallback when Safari has no DOM access and carries resulting image/page provenance through local save and provider-scoped cloud commit without inspecting hidden network traffic or raw HTML.
...
- after an explicit logout and restart, an ordinary Codex Fullwell chat can initiate Codex-owned MCP login without asking the person to enter a terminal command, and `hfj_get_context` proves reconnection on the next turn;

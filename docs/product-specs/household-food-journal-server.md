...
For native clients, the consent page's Content Security Policy may add only the exact validated `http://127.0.0.1:<ephemeral-port>` origin for either the legacy exact `/oauth/callback` path or a merged-desktop `/callback/<nonce>` path whose nonce is 1-128 URL-safe base64 characters. This exception is route-specific; credentialed, remote, query-bearing, fragment-bearing, missing-nonce, and unrelated loopback redirects do not widen it. All other pages retain the normal form destinations, and OAuth validation still requires an exact dynamically registered redirect URI before issuing a code.

...
When OpenAI app ownership verification is active, the service exposes the exact configured challenge at `/.well-known/openai-apps-challenge` as uncached plain text. The value is supplied through the encrypted deployment credential boundary and is absent from source control and normal application logs.
...
Expose one Streamable HTTP MCP endpoint at `/mcp`, advertise the server name `fullwell-cloud`, and implement the current MCP authorization specification.
...
For native clients, the consent page's Content Security Policy may add only one exact validated loopback origin: `http://127.0.0.1:<ephemeral-port>` for either the legacy exact `/oauth/callback` path or a merged-desktop `/callback/<nonce>` path whose nonce is 1-128 URL-safe base64 characters, or `http://localhost:<explicit-port>` for Claude's exact `/callback` path. This exception is route-specific; credentialed, remote, query-bearing, fragment-bearing, missing-port, missing-nonce, extra-segment, and unrelated loopback redirects do not widen it. All other pages retain the normal form destinations, and OAuth validation still requires an exact dynamically registered redirect URI before issuing a code.
...
Public identity metadata is derived from fixed route data and the configured `PUBLIC_ORIGIN`, never the request host. The canonical origin is `https://fullwell.ai`; `https://www.fullwell.ai` and the legacy `https://fullwell.souschefstudio.com` host are permanent path-and-query-preserving redirect aliases. Only indexable public pages receive this metadata. JSON-LD identifies Sous Chef Studio, Inc. as an `Organization` and Fullwell as a `WebApplication` provided by that organization; it does not claim partnership, ratings, awards, pricing, registration, or customer counts.

The same configured apex origin determines the Apple return URL, WebAuthn relying-party ID and origin, session-cookie host, OAuth issuer and endpoints, protected-resource metadata, MCP resource, and absolute email links. The server does not accept an old-host session, passkey assertion, authorization grant, or runner token as authority for the apex resource. The legacy host remains available only as a navigation redirect.
...
The gateway serializes work to one primary device with an exclusive 90-second renewable lease, at most eight open envelopes per link and 1,000 globally. Message bodies expire within seven days. One process may lease a request at most 20 times. A newly started authenticated runner may make one explicit recovery claim that resets only its own confirmed link's saturated queued requests before leasing; subsequent claims from that process cannot keep resetting the cap. This lets a repaired or upgraded runner resume previously incompatible work without an operator editing the queue while bounding a continuously failing process. A `needs_input` terminal result becomes `awaiting_user`; the next linked inbound text resumes the same envelope and local host session. When replies were gated during host execution, the next authenticated claim retries the encrypted `response_ready` result before returning new work, without requiring another inbound message. Provider delivery receipts store only a hashed delivery ID, bounded status/failure class, and timestamps.

The runner snapshot route is membership-authorized and read-only. It returns an ETag/HEAD, content hash, bounded manifest, and archive containing only the compatibility snack profile and report, all snack/ingredient/condiment/other-grocery items, current `groceries/evidence/`, legacy `snacks/evidence/`, and the format marker. Archive paths, modes, types, file counts, individual sizes, total size, hashes, and HEAD are validated. A snapshot may contain at most 20,003 files, matching the 10,000-item plus 10,000-evidence onboarding contract and three fixed compatibility files; each file remains limited to 1 MiB and the complete uncompressed snapshot remains limited to 5 MiB. The browser and runner never receive Git credentials.

...
2. Create a bucket- and prefix-restricted application key with only `listAllBucketNames`, `listBuckets`, `readBuckets`, `listFiles`, `readFiles`, `writeFiles`, `readFileRetentions`, and `writeFileRetentions`. Do not grant `deleteFiles` or `bypassGovernance`. Record the S3 endpoint and region shown for the bucket. Backblaze permits `writeFiles` to create an S3 hide marker, but removing a retained upload version still requires `deleteFiles`; recovery must list versions and download the compliance-locked upload by file ID when the current name is hidden.

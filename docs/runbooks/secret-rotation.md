...
- WhatsApp access token: disable service replies and runner claims, create a replacement least-privilege persistent token in Meta, replace only the encrypted access-token credential, restart, run a bounded provider health/send test inside an open free window, then revoke the old token.
- WhatsApp app secret: disable webhook intake, replace the app secret in Meta and the encrypted credential as one maintenance change, restart, then prove old signatures fail and a new signed test event succeeds before re-enabling intake.
- WhatsApp verify token: choose a new random value, replace the encrypted credential, restart, and update Meta's webhook subscription immediately; prove the old challenge fails and the new challenge succeeds without logging either token.
- Message encryption: stop intake and claims, drain or expire all encrypted envelopes and pending links, verify the messaging tables contain no dependent ciphertext, replace the independent key, restart, and relink. Version 1 does not retain a decrypt-old/encrypt-new key ring and must never strand live ciphertext.

...
Codex exposes one additional host-lifecycle tool, `fullwell_local_codex_grocery_audit_lifecycle`, only when its packaged local server starts with the fixed Codex adapter flag. Claude does not expose it. The tool accepts closed `begin`, `resume`, `checkpoint`, and `finish` variants and echoes only an opaque run ID, monotonic coordination revision, aggregate order counts, and one of `collecting`, `completed`, `partially_completed`, `blocked`, or `cancelled`. It never reads or writes household journal data. Only the exact successful post-tool hook binds that metadata to a hashed Codex session and current turn under private `PLUGIN_DATA`; failed calls, other tools, and later ordinary user turns are inert.

The Codex lifecycle marker is not audit authority or a durable grocery checkpoint. The applicable local journal or authenticated onboarding draft remains authoritative. Never place a user, household, store, order, food, URL, browser, credential, transcript, or other meaningful identifier in its opaque run ID. Call it only around an actual grocery order-detail traversal, after a durable checkpoint load and after each durable progress save. Before a final response, record exactly one terminal outcome. Never call it for restocking, delivery reordering, meal planning, recipe work, another Fullwell operation, or unrelated conversation.
...
| `hfj_commit_delivery_index` | Commit one provider's completed order evidence, delivery dishes, and agent-authored aggregate profile/report in `connected_audit_checkpoint` or `local_promotion` mode. | Editor, current HEAD/document/item revisions, `household_visibility_confirmed: true` only after a clear contextual response to that provider's preview, one canonical provider origin, at most 10,000 evidence records and 10,000 items within the 16 MiB request limit, and one provider-scoped idempotency key; every other provider section/citation remains exact. |

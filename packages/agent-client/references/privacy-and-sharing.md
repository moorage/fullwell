...
For every journal-authoring computer-use traversal, image provenance is limited to a credential-free HTTPS image visibly associated with the exact audited item/detail page and that page's exact HTTPS URL. Never store listing thumbnails as provenance, inspect hidden network traffic or raw HTML, broaden into unrelated image search, download or proxy image bytes, or retain screenshots. Preserve a previously valid image/page pair when refresh has no newly proven replacement. Missing or rejected images remain truthful nulls and never invalidate otherwise complete textual evidence. Local and hosted writes include the same image fields and report only aggregate captured, preserved, and skipped counts.

...
A recognized older delivery-journal format may be repaired locally without another user decision. Use only `fullwell_local_household_update` with `repair_compatibility`; never open or edit the private journal directly. The operation deterministically replaces legacy identifiers and exact references, partitions a mixed restaurant name only from the item's own exact evidence, normalizes report summaries, removes obsolete browser-label fields, validates the entire result, and writes one atomic revision. It returns counts but no food names, item IDs, provider locators, or source details. It performs no hosted read or write. Any unrecognized shape, collision, missing reference, remaining privacy violation, or unrelated corruption remains untouched.
...
Before contributing each provider, explain that its dishes, restaurant locations, private order dates/groupings, fulfillment mode, and modifiers become readable by current household members. Version 1 has no per-source erase. Revoking the browser origin or leaving the household blocks future access but does not remove already attributed Git history. Owner-confirmed household deletion removes active canonical data; encrypted backups expire under the published retention period. Interpret the user's response semantically in the current conversation: a clear `yes`, `sync it`, `go ahead`, or equivalent is enough, while ambiguity gets one natural clarification. Never require a scripted phrase or use deterministic keyword matching. A decline, silence, unrelated answer, or unresolved intent keeps that provider local or skipped and makes no hosted write.
...
For every journal-authoring computer-use traversal, image provenance is limited to a credential-free HTTPS image visibly associated with the exact audited item/detail page and that page's exact HTTPS URL. Prefer supported visible-page DOM access when it exposes the displayed image URL. When Safari Computer Use has no DOM access, an accessibility-tree omission is not proof that no image exists: target the exact visible image, use only a currently exposed context-menu action whose meaning is `Open Image in New Tab`, read the credential-free HTTPS address from that temporary tab, retain the original audited page as provenance, return to it, and close the temporary tab. Fail closed when any image, action, address, or association is unavailable or ambiguous. Never store listing thumbnails as provenance, inspect hidden network traffic or raw HTML, broaden into unrelated image search, download or proxy image bytes, retain screenshots, read clipboard contents, or guess a menu action or URL. Preserve a previously valid image/page pair when refresh has no newly proven replacement. Missing or rejected images remain truthful nulls and never invalidate otherwise complete textual evidence. Local and hosted writes include the same image fields and report only aggregate captured, preserved, and skipped counts.

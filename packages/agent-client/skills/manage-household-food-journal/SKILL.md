...
7. If the user says yes, use the cloud path. Call `hfj_get_context`; when the hosted tool is available, it starts OAuth when needed. If the `fullwell-cloud` tools are absent in Codex after a prior logout, follow **Recover a logged-out Codex cloud connection** below. Tell the user to finish in the service browser window and never request a token.
...
## Recover a logged-out Codex cloud connection

Use this recovery only after the person has explicitly chosen to connect or reconnect a Fullwell cloud account and `hfj_get_context` is unavailable because Codex does not expose the `fullwell-cloud` tools. Never use it to discover whether the person has an account, before cloud consent, for a local-only flow, or merely because an ordinary hosted tool call failed.

1. In Codex only, tell the person that Codex is opening Fullwell sign-in. Use the host command-execution tool to run exactly `codex mcp login fullwell-cloud` and wait for it to finish. Do not add flags or substitute a server name, URL, scope, command, or any user-provided value.
2. Codex owns the browser callback, token exchange, refresh credential, and keyring entry. Never request, inspect, copy, store, summarize, or echo a token, one-time code, callback parameter, authorization URL, or raw command transcript.
3. If the command reports success, say only that sign-in finished and ask the person to tell you to continue. Do not claim that the cloud household is connected yet: Codex binds MCP tools at the start of a turn.
4. On the next user turn, call `hfj_get_context` before any other hosted tool. Only its successful result proves that the cloud connection is available; then continue the interrupted cloud workflow without repeating the account question.
5. If command execution is unavailable or denied, or browser sign-in is cancelled or fails, make no hosted or local-authority change. Explain in ordinary language that Codex could not finish Fullwell sign-in and offer to retry. Do not expose raw OAuth or command errors and do not ask the person to paste or run the command themselves.
6. In Claude, never run the Codex command. Use Claude's native protected-tool authentication flow.
